...
export async function fetchNearbyFavorites(latitude: number, longitude: number, token: string):Promise<ApiResponse<FetchAllLocation>> {
  checkToken(token);
  return apiGet<FetchAllLocation>(`favoris/lire/${latitude}/${longitude}`, token);
}